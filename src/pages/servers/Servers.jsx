...
import ServerDetailPage from './ServersDetail';
...
export default function ServersPage() {
    ...
    const [networkIOInterfacesInfo, setNetworkIOInterfacesInfo] = useState([]);
    ...
    const setupWebSocket = (ipAddress) => {
        ...
        wsRef.current = new WebSocket(`ws://${ipAddress}:33551/ws/network/iocounters`);
        ...
        wsRef.current.onmessage = (event) => {
            try {
                ...
                setNetworkIOInterfacesInfo(data);
                console.log(data);
    ...
    const fetchSystemInfo = async (ipAddress) => {
        ...
        try {
            const response = await fetch(
                `http://${ipAddress}:33551/metrics/system`
            );
            if (!response.ok) {
    ...
    const handleServerClick = async (server) => {
        ...
        setNetworkIOInterfacesInfo([]); // Reset network info when selecting new server
                ...
                <Card className="w-full mb-8">
                    {selectedServer ? (
                        <ServerDetailPage 
                            ...
                            networkInfo={networkIOInterfacesInfo}
                            isLoading={isLoading}
                            error={error}
                            onBack={handleBack}
                        />
